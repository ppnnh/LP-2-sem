...
namespace Log {
	...
	void WriteIn(LOG log, In::IN in) {
		...
		*log.stream << "Строка : " << in.text << std::endl;
	...
	void WriteError(LOG log, Error::ERROR error) {
		...
		*log.stream << "Ошибка " << error.id << ":" << error.message << std::endl;
		...
		if (error.errorPosition.line != -1) { *log.stream << "Строка " << error.errorPosition.line << std::endl << "Позиция: " << error.errorPosition.col << std::endl << std::endl; }
	}
	void Close(LOG log) {
		log.stream->close();
		delete log.stream;
