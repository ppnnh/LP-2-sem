...
namespace Error
{
	ERROR errors[ERROR_MAX_ENTRY] =
	{
		...
		ERROR_ENTRY(0, "Недопустимый код ошибки"), // 0 - код ошибки вне диапазона
	...
	ERROR geterror(int id) {						 //проверяет допустимый диапазон id
		ERROR e;
	...
	ERROR geterrorin(int id, int line = -1, int col = -1) {
		...
		if (0 < id < ERROR_MAX_ENTRY) {
			e = errors[id];
			e.errorPosition.col = line;
			e.errorPosition.line = col;
			return e;
		}
		else {
			return ERROR_ENTRY(0, "");
